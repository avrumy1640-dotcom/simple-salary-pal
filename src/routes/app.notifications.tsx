...
interface Notif {
  ...
  company_id: string; entity_type: string | null; entity_id: string | null;
}

function NotificationsList({ scope, basePath }: { scope: "self" | "company"; basePath: "/app" | "/employee" }) {
  ...
  async function load() {
    ...
    if (scope === "self") q = q.eq("user_id", uid);
    const { data } = await q;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Notifications"
        description={scope === "company" ? "All notifications in your company." : "Your inbox of schedule updates and approvals."}
        actions={unread > 0 ? (
...
export const Route = createFileRoute("/app/notifications")({
  ...
  component: () => <NotificationsList scope="company" basePath="/app" />,
});
