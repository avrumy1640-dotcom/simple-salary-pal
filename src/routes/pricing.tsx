...
function PricingPage() {
  return (
    <div className="page-in relative min-h-screen overflow-hidden text-foreground">
      ...
      <div aria-hidden className="pointer-events-none absolute -left-32 top-10 h-96 w-96 rounded-full bg-[#F5C518]/80 blur-3xl orb-1" />
      <div aria-hidden className="pointer-events-none absolute -right-24 top-60 h-96 w-96 rounded-full bg-[#111827]/90 blur-3xl orb-2" />

      <header className="sticky top-0 z-30 border-b border-[#F5C518]/40/50 bg-white/80 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8">
          <Link to="/" className="flex items-center gap-2">
            <div className="grid h-9 w-9 place-items-center rounded-2xl gradient-brand text-sm font-extrabold text-white shadow-glow">P</div>
            <span className="font-display text-xl font-extrabold tracking-tight text-white">Paylo</span>
          </Link>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-24 pt-16 md:px-8 md:pt-24">
        <div className="mx-auto max-w-2xl text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-[#F5C518]/40 bg-[#111827] px-3 py-1 text-xs font-semibold text-white">
            <Sparkles className="h-3.5 w-3.5" /> Simple, transparent pricing
          </div>
          <h1 className="mt-5 font-display text-5xl font-extrabold tracking-tight text-white md:text-6xl">
          ...
          <p className="mt-2 text-sm font-medium text-slate-400">14-day free trial · No credit card required</p>
        </div>
          ...
          {tiers.map((t, i) => (
            <div
              ...
              className={[
                ...
                t.highlight
                  ? "lg:scale-[1.04] lg:my-[-12px] bg-white border-2 border-[#F5C518]/40 shadow-float hover:shadow-glow"
                  : "surface-panel hover:shadow-glow",
              ].join(" ")}
            >
              {t.highlight && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-gradient-to-r from-[#F5C518] to-[#111827] px-4 py-1.5 text-xs font-extrabold uppercase tracking-wider text-white shadow-glow">
                  Most Popular
                </div>
              )}
              <div className="text-sm font-bold uppercase tracking-[0.16em] text-slate-400">{t.name}</div>
              <div className="mt-3 flex items-baseline gap-1.5">
                <span className="font-display text-5xl font-extrabold tracking-tight text-white">{t.price}</span>
                <span className="text-sm font-semibold text-slate-400">/month</span>
              </div>
              <div className="mt-1 text-sm font-medium text-slate-400">{t.perEmp}</div>
              <p className="mt-4 text-sm text-slate-400">{t.blurb}</p>
              ...
              <Link
                ...
                className={[
                  ...
                  t.highlight
                    ...
                    : "border border-white/20/15 bg-white text-white hover:bg-[#111827] hover:shadow-soft",
                ].join(" ")}
              >
              ...
              <ul className="mt-7 space-y-3">
                {t.features.map((f) => (
                  <li key={f} className="flex items-start gap-2.5 text-sm text-white">
                    <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-[#F5C518]">
        ...
        <div className="mx-auto mt-16 max-w-3xl rounded-3xl surface-glass p-6 text-center md:p-8">
          <div className="font-display text-2xl font-bold text-white">Need something custom?</div>
